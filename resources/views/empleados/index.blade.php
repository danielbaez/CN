		...
		<div class="table-responsive">
        	<table class="table table-hover">
        		...
        		<tbody>
        			...
        			<tr>
        				<th scope="row">{{ $empleado->id }}</th><td>{{ $empleado->nombre }}</td><td>{{ $empleado->apellido }}</td><td>{{ $empleado->tipo_documento }}</td><td>{{ $empleado->nro_documento }}</td><td>{{ $empleado->telefono }}</td><td>{{ $empleado->estado == 1 ? 'Activo' : 'Inactivo' }}</td><td><img style="width: 60px;height: 60px;" class="img-circle" src="{{ asset('images/img.jpg') }}"></td><td><a href="{{ route('admin.empleados.edit', $empleado) }}" class="btn btn-primary"><i class="fa fa-edit fa-lg"></i></a><a class="btn btn-danger"><i class="fa fa-times fa-lg"></i></a></td>
        			</tr>
        			@endforeach
        		</tbody>
        	</table>
